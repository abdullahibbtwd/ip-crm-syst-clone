...
export function assigneeRoleLabel(roles: string[]) {
  if (roles.includes('trademark_attorney')) return ATTORNEY_ROLE_LABELS.trademark_attorney
  if (roles.includes('ip_attorney')) return ATTORNEY_ROLE_LABELS.ip_attorney
  if (roles.includes('managing_partner')) return ATTORNEY_ROLE_LABELS.managing_partner
