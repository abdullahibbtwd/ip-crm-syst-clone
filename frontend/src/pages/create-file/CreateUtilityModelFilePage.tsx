...
export function CreateUtilityModelFilePage() {
  ...
  const [utilityModelName, setUtilityModelName] = useState('')
  ...
  const handleSaveDraft = async () => {
    ...
    if (!utilityModelName.trim()) {
      setError(t('createFile.errors.utilityModelName'))
      return
    }
    ...
    const attributes: Record<string, unknown> = {
      utilityModelName: utilityModelName.trim(),
    ...
    try {
      ...
      if (!ownerSameAsClient && ownerLegalName.trim()) {
        const createdOwner = await createClient.mutateAsync({
          ...
          notes: 'Created as utility model owner from Create file',
          registeredLegalAddress: {
      ...
      const matter = await createMatter.mutateAsync({
        ...
        title: utilityModelName.trim(),
        status: 'draft',
        description: t('createFile.draftDescription', {
          procedure: t('createFile.kinds.utilityModel'),
        }),
    ...
    <div className="mx-auto max-w-4xl space-y-6 pb-16">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h1 className="font-serif text-3xl tracking-tight text-foreground">
                {t('createFile.utilityModelFormTitle')}
              </h1>
              <p className="mt-1 text-sm text-muted-foreground">
                {t('createFile.kinds.utilityModel')}
                {' · '}
      ...
      <SectionCard title={t('createFile.sections.basicInfo')}>
        <div className="space-y-4">
          ...
          <Field label={`${t('createFile.fields.utilityModelName')} *`}>
            <Input
              value={utilityModelName}
              onChange={(e) => setUtilityModelName(e.target.value)}
            />
          </Field>
