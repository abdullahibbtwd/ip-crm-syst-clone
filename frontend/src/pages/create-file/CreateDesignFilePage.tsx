...
import {
  DESIGN_FILING_ROUTES,
  designNeedsCountries,
  jurisdictionsForDesignRoute,
  normalizeDesignFilingRoute,
  type DesignFilingRoute,
} from '@/features/create-file/design-subtypes'
...
export function CreateDesignFilePage() {
  ...
  const [designerSameAsClient, setDesignerSameAsClient] = useState(true)
  const [designerName, setDesignerName] = useState('')
  const [designerAddress, setDesignerAddress] = useState<AddressDraft>(emptyAddress)

  const [filingRoute, setFilingRoute] = useState<DesignFilingRoute | null>(null)
  const [designCountries, setDesignCountries] = useState<string[]>([''])
  ...
  const jurisdictions = useMemo(
    () => jurisdictionsForDesignRoute(filingRoute, designCountries),
    [filingRoute, designCountries],
  )
  ...
  const handleSaveDraft = async () => {
    ...
    if (!designerSameAsClient && !designerName.trim()) {
      setError(t('createFile.errors.designerOrDetails'))
      return
    }
    if (!filingRoute) {
    ...
    if (!designName.trim()) {
      setError(t('createFile.errors.designName'))
      return
    }
    ...
    const locarno =
      [locarnoClass.trim(), locarnoSubclass.trim()].filter(Boolean).join('-') ||
      undefined

    const attributes: Record<string, unknown> = {
      designProcedure: filingRoute,
      designName: designName.trim(),
      locarnoClass: locarno || locarnoClass.trim() || undefined,
      locarnoSubclass: locarnoSubclass.trim() || undefined,
      locarnoClassNumber: locarnoClass.trim() || undefined,
      productDescription: classificationText.trim() || undefined,
      designCountries: designNeedsCountries(filingRoute)
        ? designCountries.filter(Boolean)
        : undefined,
      ...
      designerSameAsClient,
      designerName: designerSameAsClient
        ? undefined
        : designerName.trim() || undefined,
    ...
    try {
      ...
      if (!ownerSameAsClient && ownerLegalName.trim()) {
        const createdOwner = await createClient.mutateAsync({
          ...
          notes: 'Created as design owner from Create file',
          registeredLegalAddress: {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 pb-16">
      ...
      <SectionCard title={t('createFile.sections.designer')}>
        <label className="mb-4 flex items-center gap-2 text-sm">
          <input
            type="checkbox"
            checked={designerSameAsClient}
            onChange={(e) => setDesignerSameAsClient(e.target.checked)}
            className="size-4 rounded border"
          />
          {t('createFile.sameDesigner')}
        </label>
        {!designerSameAsClient ? (
          <div className="grid gap-3 sm:grid-cols-2">
            <Field
            ...
            <Field label={t('createFile.fields.city')}>
              <Input
                value={designerAddress.city}
                onChange={(e) =>
                  setDesignerAddress((a) => ({ ...a, city: e.target.value }))
                }
              />
            </Field>
            <Field label={t('createFile.fields.postalCode')}>
              <Input
                value={designerAddress.postalCode}
                onChange={(e) =>
                  setDesignerAddress((a) => ({ ...a, postalCode: e.target.value }))
                }
              />
            </Field>
            <Field label={t('createFile.fields.country')}>
              <CountrySelect
                value={designerAddress.country}
                onValueChange={(code) =>
                  setDesignerAddress((a) => ({ ...a, country: code }))
                }
              />
            </Field>
            <Field label={t('createFile.fields.address')} className="sm:col-span-2">
              <Input
                value={designerAddress.address}
                onChange={(e) =>
                  setDesignerAddress((a) => ({ ...a, address: e.target.value }))
                }
              />
            </Field>
          </div>
      ...
      <SectionCard title={t('createFile.sections.basicInfo')}>
        <div className="space-y-4">
          <div className="space-y-3">
            <div className="grid gap-3 sm:grid-cols-2">
              <Field label={`${t('createFile.fields.selectProcedure')} *`}>
                <Select
                  ...
                  onValueChange={(v) => {
                    const next = normalizeDesignFilingRoute(v)
                    setFilingRoute(next)
                    if (!designNeedsCountries(next)) setDesignCountries([''])
                  }}
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    {DESIGN_FILING_ROUTES.map((route) => (
              ...
              {designNeedsCountries(filingRoute) ? (
                <Field label={t('createFile.fields.country')}>
                  <Select
                    value={designCountries[0] || undefined}
                    onValueChange={(v) =>
                      setDesignCountries((rows) => {
                        const next = [...rows]
                        next[0] = v ?? ''
                        return next
                      })
            ...
            {designNeedsCountries(filingRoute) ? (
              <div className="space-y-2">
                {designCountries.slice(1).map((code, extraIndex) => {
                  const index = extraIndex + 1
                  return (
                    <div key={`design-country-${index}`} className="flex items-end gap-2">
                      <Field
                        label={t('createFile.fields.country')}
                ...
                <Button
                  ...
                  variant="outline"
                  size="icon"
                  onClick={() => setDesignCountries((rows) => [...rows, ''])}
                  aria-label={t('createFile.fields.addCountry')}
                >
          ...
          <Field label={`${t('createFile.fields.designName')} *`}>
            <Input
              value={designName}
      ...
      <SectionCard title={t('createFile.sections.classification')}>
        <div className="grid gap-3 sm:grid-cols-[120px_140px_1fr]">
          <Field label={t('createFile.fields.locarnoClass')}>
            <Input
              value={locarnoClass}
              onChange={(e) => setLocarnoClass(e.target.value)}
            />
          </Field>
          <Field label={t('createFile.fields.locarnoSubclass')}>
            <Input
              value={locarnoSubclass}
              onChange={(e) => setLocarnoSubclass(e.target.value)}
            />
          </Field>
          <Field label={t('createFile.fields.classificationText')}>
            <Input
