...
export function CancellationNotesTab() {
  ...
  const notes = readCancellationFields(matter).notes
  ...
  const persistNotes = async (nextNotes: CancellationNote[]) => {
    await updateMatter.mutateAsync({
      attributes: {
        ...attrs,
        cancellationNotes: nextNotes,
      },
    })
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null)
    if (!regarding.trim()) {
      setError(t('cancellationView.notes.regardingRequired'))
      return
    }
    ...
    try {
      ...
      setError(getApiErrorMessage(err, t('cancellationView.notes.saveFailed')))
    }
  }

  const startEdit = (note: CancellationNote) => {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4">
      <Card>
        <CardHeader>
          <CardTitle className="text-base">{t('cancellationView.notes.title')}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          {notes.length === 0 ? (
            <p className="text-sm text-muted-foreground">{t('cancellationView.notes.empty')}</p>
          ) : (
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>{t('cancellationView.notes.date')}</TableHead>
                  <TableHead>{t('cancellationView.notes.regarding')}</TableHead>
                  <TableHead>{t('cancellationView.notes.details')}</TableHead>
                      ...
                      <TableCell>
                        <div className="flex justify-end gap-1">
                          <Button
                            ...
                            aria-label={t('cancellationView.notes.editAria')}
                          >
                          ...
                          <Button
                            ...
                            aria-label={t('cancellationView.notes.deleteAria')}
                          >
            ...
            >
              <label className="block space-y-1.5 text-sm">
                <span className="text-xs text-muted-foreground uppercase tracking-wide">
                  {t('cancellationView.notes.regarding')}
                </span>
                <Input value={regarding} onChange={(e) => setRegarding(e.target.value)} />
              </label>
              <label className="block space-y-1.5 text-sm">
                <span className="text-xs text-muted-foreground uppercase tracking-wide">
                  {t('cancellationView.notes.details')}
                </span>
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button type="submit" disabled={updateMatter.isPending}>
                  {updateMatter.isPending
                    ...
                    : editingId
                      ? t('cancellationView.save')
                      : t('cancellationView.notes.submit')}
                </Button>
              </div>
            </form>
          ) : null}
