...
import {
  designStatusLabel,
  designTerritoryLabel,
  formatLocarnoClassification,
  formatRefNumberDate,
} from '@/features/matters/design-list-utils'
import { DEFAULT_PAGE_SIZE } from '@/lib/pagination'
import { cn } from '@/lib/utils'

export const DESIGN_PAGE_SIZE = DEFAULT_PAGE_SIZE
...
function ownerName(matter: MatterListItem): string {
  const summary = matter.designSummary
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-border/80 bg-muted/50 hover:bg-muted/50">
            <TableHead className="min-w-[160px]">{t('designList.columns.name')}</TableHead>
            <TableHead className="min-w-[100px]">{t('designList.columns.territory')}</TableHead>
            <TableHead className="min-w-[130px]">{t('designList.columns.incomingRef')}</TableHead>
            <TableHead className="min-w-[120px]">{t('designList.columns.status')}</TableHead>
            <TableHead className="min-w-[110px]">{t('designList.columns.classification')}</TableHead>
            <TableHead className="min-w-[140px]">{t('designList.columns.owner')}</TableHead>
            <TableHead className="min-w-[120px]">{t('designList.columns.registration')}</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
              ...
              return (
                ...
                >
                  ...
                  <TableCell className="text-sm">
                    {designTerritoryLabel(summary?.territoryCode ?? null)}
                  </TableCell>
                  ...
                  <TableCell>
                    <Badge variant="info" className="normal-case font-medium tracking-normal">
                      {designStatusLabel(summary ?? null)}
                    </Badge>
                  </TableCell>
                  <TableCell className="whitespace-normal text-sm font-medium tabular-nums">
                    {formatLocarnoClassification(summary ?? null)}
                  </TableCell>
