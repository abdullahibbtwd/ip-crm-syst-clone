...
import { formatRefNumberDate } from '@/features/matters/utility-model-list-utils'
...
type UtilityModelInfoFieldsProps = {
  matter: MatterDetail
}
...
export function UtilityModelInfoFields({ matter }: UtilityModelInfoFieldsProps) {
  const { t } = useTranslation('matters')

  if (matter.matterType !== 'utility_model') return null
  ...
  const modelName =
    (typeof attrs.utilityModelName === 'string' && attrs.utilityModelName.trim()) ||
    matter.title

  return (
    <Card className="lg:col-span-2">
      <CardHeader>
        <CardTitle className="text-base">{t('utilityModelInfo.title')}</CardTitle>
        <p className="text-xs text-muted-foreground">
          {t('utilityModelInfo.subtitle', {
      ...
      <CardContent className="space-y-4">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <InfoCell
            label={t('utilityModelInfo.fields.application')}
            primary={application.primary}
            secondary={application.secondary}
          />
          <InfoCell
            label={t('utilityModelInfo.fields.registration')}
            primary={registration.primary}
            secondary={registration.secondary}
          />
          <InfoCell
            label={t('utilityModelInfo.fields.bulletin')}
            primary={bulletin.primary}
            secondary={bulletin.secondary}
          />
          <InfoCell label={t('utilityModelInfo.fields.ipc')} primary={readIpc(attrs)} />
        </div>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <InfoCell label={t('utilityModelInfo.fields.applicant')} primary={clientName} />
          <InfoCell label={t('utilityModelInfo.fields.inventor')} primary={inventorName} />
          <InfoCell label={t('utilityModelInfo.fields.contact')} primary={readContact(attrs)} />
          <InfoCell
            label={t('utilityModelInfo.fields.representatives')}
            primary={readRepresentatives(attrs)}
          />
          <InfoCell label={t('utilityModelInfo.fields.claims')} primary={readClaims(attrs)} />
          <InfoCell label={t('utilityModelInfo.fields.territory')} primary={territoryLabel} />
        </div>
      </CardContent>
    </Card>
  )
