...
export const DELETIONS_PAGE_SIZE = DEFAULT_PAGE_SIZE

type DeletionsTableProps = {
        ...
        <TableHeader>
          <TableRow className="border-border/80 bg-muted/50 hover:bg-muted/50">
            <TableHead className="min-w-[140px]">
              {t('deletionList.columns.againstMark')}
            </TableHead>
            <TableHead className="min-w-[160px]">
              {t('deletionList.columns.client')}
            </TableHead>
            <TableHead className="min-w-[140px]">
              {t('deletionList.columns.status')}
            </TableHead>
            <TableHead className="min-w-[140px] text-right">
              {t('deletionList.columns.actions')}
            </TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
                ...
                <div className="mx-auto flex max-w-sm flex-col items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">
                      {t('deletionList.emptyDescription')}
                    </p>
                  </div>
                </div>
              </TableCell>
          ...
          {!isLoading &&
            ...
            items.map((matter, index) => {
              const stage = matter.trademarkSummary?.deletionStage
              ...
              return (
                ...
                >
                  ...
                  <TableCell>
                    <DeletionStageBadge stage={stage} matterStatus={matter.status} />
                  </TableCell>
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-1">
                      <Button
                        ...
                        aria-label={t('deletionList.viewAria', { title: matter.title })}
                        title={t('deletionList.view')}
                      >
                      ...
                      <Button
                        ...
                        onClick={() => navigate(`/matters/${matter.id}/deletion-archive`)}
                        aria-label={t('deletionList.filesAria', {
                          title: matter.title,
                          count: docCount,
                        })}
                        title={t('deletionList.fileArchive')}
                      >
