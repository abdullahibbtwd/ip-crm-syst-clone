...
import {
  ...
  spcStatusLabel,
  spcTerritoryLabel,
} from '@/features/matters/spc-list-utils'
import { DEFAULT_PAGE_SIZE } from '@/lib/pagination'
import { cn } from '@/lib/utils'

export const SPC_PAGE_SIZE = DEFAULT_PAGE_SIZE
...
function ownerName(matter: MatterListItem): string {
  const summary = matter.spcSummary
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-border/80 bg-muted/50 hover:bg-muted/50">
            <TableHead className="min-w-[160px]">{t('spcList.columns.name')}</TableHead>
            <TableHead className="min-w-[100px]">{t('spcList.columns.territory')}</TableHead>
            <TableHead className="min-w-[130px]">{t('spcList.columns.incomingRef')}</TableHead>
            <TableHead className="min-w-[120px]">{t('spcList.columns.status')}</TableHead>
            <TableHead className="min-w-[90px]">{t('spcList.columns.classes')}</TableHead>
            <TableHead className="min-w-[140px]">{t('spcList.columns.owner')}</TableHead>
            <TableHead className="min-w-[90px]">{t('spcList.columns.kind')}</TableHead>
            <TableHead className="min-w-[120px]">{t('spcList.columns.registration')}</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
              ...
              return (
                ...
                >
                  ...
                  <TableCell className="text-sm">
                    {spcTerritoryLabel(summary?.territoryCode ?? null)}
                  </TableCell>
