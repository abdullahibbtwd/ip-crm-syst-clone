...
import {
  formatRefNumberDate,
  formatUtilityModelClaims,
  utilityModelStatusLabel,
  utilityModelTerritoryLabel,
} from '@/features/matters/utility-model-list-utils'
import { DEFAULT_PAGE_SIZE } from '@/lib/pagination'
import { cn } from '@/lib/utils'

export const UTILITY_MODEL_PAGE_SIZE = DEFAULT_PAGE_SIZE

type UtilityModelsTableProps = {
...
function ownerName(matter: MatterListItem): string {
  const summary = matter.utilityModelSummary
...
export function UtilityModelsTable({
  items,
  isLoading,
  isError,
  page,
      ...
      <Table>
        <TableHeader>
          <TableRow className="border-border/80 bg-muted/50 hover:bg-muted/50">
            <TableHead className="min-w-[160px]">{t('utilityModelList.columns.name')}</TableHead>
            <TableHead className="min-w-[100px]">{t('utilityModelList.columns.territory')}</TableHead>
            <TableHead className="min-w-[130px]">{t('utilityModelList.columns.incomingRef')}</TableHead>
            <TableHead className="min-w-[120px]">{t('utilityModelList.columns.status')}</TableHead>
            <TableHead className="min-w-[90px]">{t('utilityModelList.columns.classes')}</TableHead>
            <TableHead className="min-w-[140px]">{t('utilityModelList.columns.owner')}</TableHead>
            <TableHead className="min-w-[90px]">{t('utilityModelList.columns.kind')}</TableHead>
            <TableHead className="min-w-[120px]">{t('utilityModelList.columns.registration')}</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
                ...
                <div className="mx-auto flex max-w-sm flex-col items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">
                      {t('utilityModelList.emptyDescription')}
                    </p>
                  </div>
                </div>
              </TableCell>
              ...
              return (
                ...
                >
                  ...
                  <TableCell className="text-sm">
                    {utilityModelTerritoryLabel(summary?.territoryCode ?? null)}
                  </TableCell>
                  ...
                  <TableCell>
                    <Badge variant="info" className="normal-case font-medium tracking-normal">
                      {utilityModelStatusLabel(summary ?? null)}
                    </Badge>
                  </TableCell>
                  <TableCell className="whitespace-normal text-sm font-medium tabular-nums">
                    {formatUtilityModelClaims(summary ?? null)}
                  </TableCell>
