...
export type UtilityModelListFilterInput = {
...
export function utilityModelListFilterWhere(
  query: UtilityModelListFilterInput,
): Prisma.MatterWhereInput | undefined {
  const clauses: Prisma.MatterWhereInput[] = [];

  const applicant = trim(query.utilityModelApplicant);
  if (applicant) {
    clauses.push({
      OR: [
  ...
  const name = trim(query.utilityModelName);
  if (name) {
    clauses.push({
      OR: [
        { title: { contains: name, mode: 'insensitive' } },
        jsonAttrStringContains(['utilityModelName'], name),
      ],
    });
  }

  const incoming = trim(query.utilityModelIncoming);
  if (incoming) {
    clauses.push({
      OR: [
  ...
  const regNo = trim(query.utilityModelRegNo);
  if (regNo) {
    clauses.push({
      OR: [
  ...
  const territory = trim(query.utilityModelTerritory)?.toUpperCase();
  if (territory) {
    clauses.push({
      OR: [
  ...
  const representative = trim(query.utilityModelRepresentative);
  if (representative) {
  ...
  const appFrom = trim(query.utilityModelAppFrom);
  const appTo = trim(query.utilityModelAppTo);
  if (appFrom || appTo) {
  ...
  const regFrom = trim(query.utilityModelRegFrom);
  const regTo = trim(query.utilityModelRegTo);
  if (regFrom || regTo) {
  ...
  const contact = trim(query.utilityModelContact);
  if (contact) {
    clauses.push({
      OR: [
        {
  ...
  const stage = trim(query.utilityModelStage);
  if (stage) {
    if (stage === 'registered') {
      clauses.push({
        OR: [
  ...
  const certificate = trim(query.utilityModelCertificate);
  if (certificate === 'with') {
    clauses.push({
